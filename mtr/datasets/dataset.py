...
class DatasetTemplate(torch_data.Dataset):
    ...
    def collate_batch(self, batch_list):
        ...
        for key, val_list in key_to_list.items():
            ...
            elif key in ['obj_types', 'obj_ids', 'center_objects_type', 'center_objects_id']:
                input_dict[key] = np.concatenate(val_list, axis=0)
            elif key in ['scenario_id']:
                input_dict[key] = np.array(val_list)
            elif key in ['bev_data']:
                from mmcv.parallel import collate
                input_dict[key] = collate(val_list)
                input_dict[key]['img_metas'] = [a[0] for a in input_dict[key]['img_metas'].data]
                input_dict[key]['img'] = torch.cat(input_dict[key]['img'].data,dim=0)
            else:
                # val_list = [torch.from_numpy(x) for x in val_list]
                # print(key,val_list[0].shape,val_list[1].shape)
