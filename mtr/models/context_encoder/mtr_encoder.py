...
class MTREncoder(nn.Module):
    def __init__(self, config):
        ...
        # build bev feature max pooling layer
        if self.model_cfg.USE_BEV:
            self.bev_pool =  nn.MaxPool2d(kernel_size=10)
        ...
        for _ in range(self.model_cfg.NUM_ATTN_LAYERS):
            self_attn_layers.append(self.build_transformer_encoder_layer(
                d_model=self.model_cfg.D_MODEL if not self.model_cfg.USE_BEV else self.model_cfg.D_MODEL*2,
        ...
        self.num_out_channels = self.model_cfg.D_MODEL if not self.model_cfg.USE_BEV else self.model_cfg.D_MODEL*2
    ...
    def forward(self, batch_dict):
        ...
        if self.model_cfg.USE_BEV:
            bev_features = input_dict['bev_features'].cuda()
            # print(bev_features.shape)
            # print(bev_features.grad)
            
            bev_features = self.bev_pool(bev_features).squeeze(dim=2).squeeze(dim=2)
            global_bev_feature = bev_features[:,None,:].repeat(1,num_objects+num_polylines,1)
            global_token_feature = torch.cat((global_token_feature, global_bev_feature), dim=2)
            # print(global_token_feature.shape)
            # print(global_bev_feature.shape)
            # print(global_token_feature.shape)
            # print("="*30)
